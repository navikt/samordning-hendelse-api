...
object TestTokenHelper {
    ...
    fun token(scope: String, orgno: String, verifiedSignature: Boolean): String {
        val signingKeys = if (verifiedSignature) keyPair else generateKeyPair()
    ...
    @Throws(NoSuchAlgorithmException::class)
    private fun generateKeyPair() = KeyPairGenerator.getInstance("RSA").run {
        initialize(1024)
        generateKeyPair()
    }
    ...
    private fun expiredJwt() =
            JWT.create().withExpiresAt(GregorianCalendar(2018, DECEMBER, 31).time)

    private fun futureJwt() =
            JWT.create().withNotBefore(GregorianCalendar(2101, JANUARY, 1).time)
    ...
    private fun algorithm() = Algorithm.RSA256(publicKey(keyPair), privateKey(keyPair))
